...
bool durum2 = false;
// byte Image [256];
void testdrawbitmap(byte bitmap[maxaddress])
{
...
void sdKartOku()
{
  ...
  //File dataFile = sd.open("eepromsil.txt");
    ...
    while (dataFile.available())
    {
      ...
      if (c != ' ' && c != '{' && c != ',' && c != '\n' && c != '\r' && c != '}')
      {
        ...
        if ((diziindex) % 4 == 3)
        {
          ...
          ram_array[index2 - (page * maxaddress)]=strtol(data, NULL, 16);
